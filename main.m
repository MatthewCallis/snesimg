	...
	@autoreleasepool {
		...
		if(![sourceImage isValid]){
			...
			if(debug){
				...
				NSLog(@"Size:  %ld px wide", (long)width);
				NSLog(@"Size:  %ld px high", (long)height);
				NSLog(@"Bits:  %ld bpp", (long)[bitmapData bitsPerSample]);
				NSLog(@"Bytes: %ld", (long)[bitmapData samplesPerPixel]);
				NSLog(@"Color Space: %@", [bitmapData colorSpace]);
				NSLog(@"Colors: %lu", (unsigned long)[bitmapData colorCount]);
			}
				...
				for(x = 0; x < realWidth; x++){
					...
					if(![colorCounter containsObject:[brep colorAtX:x y:y]]){
						if([brep colorAtX:x y:y]){
							if(debug) NSLog(@"Color %ld: %@", (long)colorsUsed, [[brep colorAtX:x y:y] hexString]);
